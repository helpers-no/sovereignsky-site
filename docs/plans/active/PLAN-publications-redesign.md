...
- [ ] 1.4 Add `weight` field to all publications in JSON. FFI publication as featured (lowest weight).
- [ ] 1.5 Featured publication: horizontal card (same as blog/projects featured) with image, type badge, publisher, title, description, "Read Publication" link
- [ ] 1.6 Remaining publications in compact grid (keep current compact layout, restyle with Stitch tokens): image thumbnail, type badge, publisher, title. No description — compact style.
- [ ] 1.7 Filter JavaScript with URL persistence using `.sd-filter-pill-active`
- [ ] 1.8 Sovereignsky footer, hide default footer
